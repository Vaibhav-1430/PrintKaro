...
function renderActions(item) {
  ...
  if (item.state === 'done') {
    ...
    setStatus(m ? `${m.pageCount} page(s) · ${m.paperSize} · ${m.orientation}` : 'Validated');
...
function addFiles(files) {
  ...
  for (const file of files) {
    ...
    const id = ++seq;
    const item = { id, file, state: local ? 'error' : 'queued', error: local, result: null, aborted: false };
...
function refreshContinue() {
  ...
  if (count)
    count.textContent = queue.size
      ? `${done.length} of ${queue.size} ready`
      : '';
...
async function onContinue(e) {
  ...
  store.set(CONFIG.KEYS.upload, { uploadId: first.id, filename: first.originalFilename });
  await pruneExcept(first.id);
  trackFunnel('upload_done', { count: done.length });
...
function wireDropzone() {
  ...
  if (folderInput)
    folderInput.addEventListener('change', () => folderInput.files.length && addFiles([...folderInput.files]));
